...
Perfected Alignment, Typography, and Bold Hierarchy:
- Dark Obsidian background (#050505)
- Full-bleed 0.45" Cyber Connector Grid (#1E1E1E)
- Section Tag: Consolas 8.5pt #FFD000
- Slide Title: Arial Narrow 32pt bold #F5F5F0
- Accent Underline Bar: 0.72" x 0.06" #FFD000
- Subtitle: Arial 12pt #9B9B96
- Perfect box alignment: Content starts at top=2.70", height=4.15" (or 3.20" for lower rows)
- Phone mockups match the exact top (2.70") and bottom (6.85") of adjacent cards
- High-contrast bold lead-ins for all bullets and statistics
- No dead empty voids inside cards
- All text preserved 100% identically from Final pptx
"""
...
COLOR_INNER_FILL  = RGBColor(24, 24, 24)    # #181818 Subtle Inner Widget Surface
...
def build_presentation():
    ...
    def create_base_slide(section_tag="", slide_title="", subtitle=""):
        ...
        # 3. Header Box
        if slide_title:
            tb_tag = slide.shapes.add_textbox(Inches(0.65), Inches(0.42), Inches(8.0), Inches(0.22))
            ...
            tb_title = slide.shapes.add_textbox(Inches(0.65), Inches(0.68), Inches(11.8), Inches(1.10))
            ...
            p_ttl.font.size = Pt(31)
            p_ttl.font.bold = True
            p_ttl.font.color.rgb = COLOR_TEXT_WHITE

            bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.65), Inches(1.88), Inches(0.72), Inches(0.06))
            bar.fill.solid()
            bar.fill.fore_color.rgb = COLOR_GOLD
            bar.line.fill.background()

            if subtitle:
                tb_sub = slide.shapes.add_textbox(Inches(0.65), Inches(2.05), Inches(11.8), Inches(0.48))
    ...
    # Exactly aligned to the given height and top
    def place_phone_mockup(slide, img_path, left, top, height=Inches(4.15), tag="iQOO 15 VISION SCREEN"):
        ...
        # Reserve 0.32" at bottom for tag
        img_h = height - Inches(0.32)
        w = int(img_h * aspect)
        pad = Inches(0.05)

        # Outer Frame
        create_card(slide, left, top, w + (pad * 2), height, with_corner_accent=True)

        # Image
        slide.shapes.add_picture(img_path, left + pad, top + pad, width=w, height=img_h)

        # Bottom Caption Badge
        tb = slide.shapes.add_textbox(left, top + height - Inches(0.26), w + (pad * 2), Inches(0.20))
        tf = tb.text_frame
        tf.word_wrap = True
    ...
    CONTENT_H   = Inches(4.15)
    C_LEFT_W    = Inches(8.50)
    MOCKUP_X    = Inches(9.45)
    ...
    tb1_tag = s1.shapes.add_textbox(Inches(0.65), Inches(0.48), Inches(7.5), Inches(0.22))
    ...
    tb1_m = s1.shapes.add_textbox(Inches(0.65), Inches(0.85), Inches(7.0), Inches(0.95))
    ...
    tb1_tagline = s1.shapes.add_textbox(Inches(0.65), Inches(2.05), Inches(8.0), Inches(1.20))
    ...
    tb1_desc = s1.shapes.add_textbox(Inches(0.68), Inches(3.45), Inches(8.2), Inches(1.05))
    ...
    tb1_chips = s1.shapes.add_textbox(Inches(0.68), Inches(4.70), Inches(8.2), Inches(2.10))
    ...
    # Phone Mockup (aligned vertically to span 1.10" to 6.85")
    place_phone_mockup(s1, IMG_HOME, Inches(9.45), Inches(1.10), height=Inches(5.75), tag="SIGNIFY HOME INTERFACE")

    # =========================================================================
    # SLIDE 2: 01 // THE URGENCY (3 Equal Cards with Balanced Padding)
    # =========================================================================
    ...
    metrics_data = [
        ...
        ("2-4 HRS", "INTERPRETER ARRIVAL DELAY", [
            ("Certified interpreters", "are rarely on-site when needed."),
    ...
    for i, (stat, title, bullets) in enumerate(metrics_data):
        ...
        create_card(s2, c_left, CONTENT_TOP, c_w3, CONTENT_H)

        tb = s2.shapes.add_textbox(c_left + Inches(0.32), CONTENT_TOP + Inches(0.30), c_w3 - Inches(0.64), CONTENT_H - Inches(0.50))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = tf.margin_right = tf.margin_bottom = 0
        ...
        p_ttl.text = title
        p_ttl.font.name = "Arial Narrow"
        p_ttl.font.size = Pt(14)
        p_ttl.font.bold = True
        p_ttl.font.color.rgb = COLOR_TEXT_WHITE
        p_ttl.space_after = Pt(20)
    ...
    # =========================================================================
    # SLIDE 3: 02 // CORE NOVELTY (2 Comparison Cards with Visual Chips)
    # =========================================================================
    ...
    c_w_half = Inches(5.82)
    gap_half = Inches(0.39)

    # Left Card: Standard Classifiers
    c_left_x = Inches(0.65)
    create_card(s3, c_left_x, CONTENT_TOP, c_w_half, CONTENT_H)

    tbl = s3.shapes.add_textbox(c_left_x + Inches(0.35), CONTENT_TOP + Inches(0.30), c_w_half - Inches(0.70), CONTENT_H - Inches(0.50))
    tfl = tbl.text_frame
    tfl.word_wrap = True
    tfl.margin_left = tfl.margin_top = tfl.margin_right = tfl.margin_bottom = 0
    ...
    pl_flow.text = "FLOW: Camera ──► Classifier ──► Dumps Raw Words"
    pl_flow.font.name = "Consolas"
    pl_flow.font.size = Pt(9)
    pl_flow.font.bold = True
    ...
    create_card(s3, c_right_x, CONTENT_TOP, c_w_half, CONTENT_H)

    tbr = s3.shapes.add_textbox(c_right_x + Inches(0.35), CONTENT_TOP + Inches(0.30), c_w_half - Inches(0.70), CONTENT_H - Inches(0.50))
    tfr = tbr.text_frame
    tfr.word_wrap = True
    tfr.margin_left = tfr.margin_top = tfr.margin_right = tfr.margin_bottom = 0
    ...
    pr_box.space_after = Pt(16)

    advantages = [
        ("Medical triage:", "[ SICK ] + [ OWIE ] → \"I am sick and experiencing severe pain.\""),
        ("100% Deterministic:", "Zero-Hallucination, never invents non-existent words."),
        ("Fluent communication:", "Human-first grammar synthesis in < 2ms."),
        ("Offline execution:", "Zero dependency on high-latency cloud LLMs.")
    ]
    for lead, rest in advantages:
        ...
        pa.space_after = Pt(10)

    # =========================================================================
    # SLIDE 4: 03 // PRODUCT ARCHITECTURE (2 Flow Cards + Perfectly Aligned Phone)
    # =========================================================================
    ...
    card_sub_h = Inches(1.98)
    gap_sub    = Inches(0.19)

    # Forward Direction Card
    create_card(s4, Inches(0.65), CONTENT_TOP, C_LEFT_W, card_sub_h)
    tbf = s4.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.20), C_LEFT_W - Inches(0.60), card_sub_h - Inches(0.35))
    tff = tbf.text_frame
    tff.word_wrap = True
    ...
    pf_pipe.font.size = Pt(9)
    pf_pipe.font.bold = True
    pf_pipe.font.color.rgb = COLOR_TEXT_WHITE
    pf_pipe.space_after = Pt(8)
    ...
    pf_desc.font.size = Pt(10.5)
    pf_desc.font.color.rgb = COLOR_TEXT_MUTED

    # Reverse Direction Card
    top_r = CONTENT_TOP + card_sub_h + gap_sub
    create_card(s4, Inches(0.65), top_r, C_LEFT_W, card_sub_h)
    tbr = s4.shapes.add_textbox(Inches(0.95), top_r + Inches(0.20), C_LEFT_W - Inches(0.60), card_sub_h - Inches(0.35))
    tfr = tbr.text_frame
    tfr.word_wrap = True
    tfr.margin_left = tfr.margin_top = tfr.margin_right = tfr.margin_bottom = 0
    ...
    pr_pipe.font.size = Pt(9)
    pr_pipe.font.bold = True
    pr_pipe.font.color.rgb = COLOR_TEXT_WHITE
    pr_pipe.space_after = Pt(8)
    ...
    pr_desc.font.size = Pt(10.5)
    pr_desc.font.color.rgb = COLOR_TEXT_MUTED

    # Phone Mockup (aligned to CONTENT_TOP and CONTENT_H)
    place_phone_mockup(s4, IMG_HOME, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="TWO-WAY BRIDGE HERO")

    # =========================================================================
    # SLIDE 5: 04 // LIVE PROTOTYPE REALITY (Vision Pipeline + Aligned Mockup)
    # =========================================================================
    ...
    create_card(s5, Inches(0.65), CONTENT_TOP, C_LEFT_W, CONTENT_H)
    tbp = s5.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.25), C_LEFT_W - Inches(0.60), CONTENT_H - Inches(0.40))
    tfp = tbp.text_frame
    tfp.word_wrap = True
    ...
    for stitle, sdesc in psteps:
        ...
        run.font.size = Pt(10)
        run.font.color.rgb = COLOR_TEXT_WHITE
        pst.space_after = Pt(8)

    place_phone_mockup(s5, IMG_CAMERA_SIGN, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="LIVE VISION TRACKING")

    # =========================================================================
    # SLIDE 6: 05 // TRUST & CLINICAL SAFETY (Top Bar + 2 Gate Cards)
    # =========================================================================
    ...
    c_bar_h = Inches(0.70)
    create_card(s6, Inches(0.65), CONTENT_TOP, c_bar_w, c_bar_h, with_corner_accent=False)
    tbg = s6.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.18), c_bar_w - Inches(0.60), Inches(0.40))
    tfg = tbg.text_frame
    tfg.word_wrap = True
    ...
    pg_flow.font.size = Pt(10)
    pg_flow.font.bold = True
    pg_flow.font.color.rgb = COLOR_GOLD

    gate_top = CONTENT_TOP + c_bar_h + Inches(0.20)
    gate_h   = CONTENT_H - c_bar_h - Inches(0.20)

    # Left: Accepted
    create_card(s6, Inches(0.65), gate_top, c_w_half, gate_h)
    tbg1 = s6.shapes.add_textbox(Inches(0.95), gate_top + Inches(0.25), c_w_half - Inches(0.60), gate_h - Inches(0.40))
    ...
    pga2.font.size = Pt(16)
    pga2.font.bold = True
    pga2.font.color.rgb = COLOR_TEXT_WHITE
    pga2.space_after = Pt(12)
    ...
    tbg2 = s6.shapes.add_textbox(c_right_x + Inches(0.30), gate_top + Inches(0.25), c_w_half - Inches(0.60), gate_h - Inches(0.40))
    ...
    pgb2.font.size = Pt(16)
    pgb2.font.bold = True
    pgb2.font.color.rgb = COLOR_TEXT_WHITE
    pgb2.space_after = Pt(12)

    rej_bullets = [
        ("Suppresses ambiguous transitions:", "Filters 52% blurred hand motions."),
    ...
    # =========================================================================
    # SLIDE 7: 06 // MEANINGFUL SENTENCE SYNTHESIS (Engine Card + Aligned Phone)
    # =========================================================================
    ...
    create_card(s7, Inches(0.65), CONTENT_TOP, C_LEFT_W, CONTENT_H)
    tbc = s7.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.25), C_LEFT_W - Inches(0.60), CONTENT_H - Inches(0.40))
    tfc = tbc.text_frame
    tfc.word_wrap = True
    ...
    pce1.font.size = Pt(18)
    pce1.font.bold = True
    pce1.font.color.rgb = COLOR_TEXT_WHITE
    pce1.space_after = Pt(14)
    ...
    place_phone_mockup(s7, IMG_WAVEFORM, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="AUDIO BROADCAST & WAVEFORM")

    # =========================================================================
    # SLIDE 8: 07 // LIVE TWO-WAY DIALOGUE (Timeline Card + Aligned Phone)
    # =========================================================================
    ...
    create_card(s8, Inches(0.65), CONTENT_TOP, C_LEFT_W, CONTENT_H)
    tbch = s8.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.25), C_LEFT_W - Inches(0.60), CONTENT_H - Inches(0.40))
    ...
    pch1.font.size = Pt(18)
    pch1.font.bold = True
    pch1.font.color.rgb = COLOR_TEXT_WHITE
    pch1.space_after = Pt(14)
    ...
    place_phone_mockup(s8, IMG_CHAT, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="CONVERSATION MODE STREAM")

    # =========================================================================
    # SLIDE 9: 08 // REVERSE VOICE INGEST (Voice Features Card + Aligned Phone)
    # =========================================================================
    ...
    create_card(s9, Inches(0.65), CONTENT_TOP, C_LEFT_W, CONTENT_H)
    tbm = s9.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.25), C_LEFT_W - Inches(0.60), CONTENT_H - Inches(0.40))
    tfm = tbm.text_frame
    tfm.word_wrap = True
    ...
    place_phone_mockup(s9, IMG_MIC_TRIGGER, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="TAP TO SPEAK INTERFACE")

    # =========================================================================
    # SLIDE 10: 09 // HIGH-CONTRAST ACCESSIBILITY (Readout Features + Aligned Phone)
    # =========================================================================
    ...
    create_card(s10, Inches(0.65), CONTENT_TOP, C_LEFT_W, CONTENT_H)
    tbrd = s10.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.25), C_LEFT_W - Inches(0.60), CONTENT_H - Inches(0.40))
    ...
    prd1.font.size = Pt(18)
    prd1.font.bold = True
    prd1.font.color.rgb = COLOR_TEXT_WHITE
    prd1.space_after = Pt(14)
    ...
    place_phone_mockup(s10, IMG_READOUT, MOCKUP_X, CONTENT_TOP, height=CONTENT_H, tag="HIGH-CONTRAST SPEECH CARD")

    # =========================================================================
    # SLIDE 11: 10 // SNAPDRAGON HEXAGON NPU BENCHMARK (Full & Balanced)
    # =========================================================================
    ...
    create_card(s11, Inches(0.65), CONTENT_TOP, c_bar_w, Inches(0.75), with_corner_accent=False)
    tb_npu = s11.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.12), c_bar_w - Inches(0.60), Inches(0.55))
    ...
    pn_sub.font.size = Pt(10)
    pn_sub.font.color.rgb = COLOR_TEXT_WHITE

    bench_data = [
        ("INFERENCE LATENCY", "< 18 ms", [
            ("vs. CPU: 145 ms", False),
            ("vs. Cloud: 280 ms", False),
            ("Real-time 60 FPS sub-frame speed.", True)
        ]),
        ("POWER EFFICIENCY", "0.42 W", [
            ("vs. CPU: 3.8 W", False),
            ("No phone overheating.", True),
            ("All-day continuous operation.", True)
        ]),
        ("BANDWIDTH COST", "0 KB/s", [
            ("vs. Cloud: 120 KB/s/frame", False),
            ("100% offline.", True),
            ("Works in elevators & flights.", True)
        ]),
        ("DATA PRIVACY", "100% LOCAL", [
            ("Zero video leaves device.", True),
            ("Full HIPAA compliance.", True),
            ("Zero cloud leak risk.", True)
        ])
    ]
    c_w4 = Inches(2.80)
    gap4 = Inches(0.27)
    card_top4 = CONTENT_TOP + Inches(0.75) + Inches(0.18)
    card_h4   = CONTENT_H - Inches(0.75) - Inches(0.18)

    for i, (mtitle, mval, mitems) in enumerate(bench_data):
        c_x = Inches(0.65) + i * (c_w4 + gap4)
        create_card(s11, c_x, card_top4, c_w4, card_h4)

        tb_m = s11.shapes.add_textbox(c_x + Inches(0.25), card_top4 + Inches(0.25), c_w4 - Inches(0.50), card_h4 - Inches(0.40))
        ...
        for line, is_hl in mitems:
            pl = tf_m.add_paragraph()
            pl.text = line
            pl.font.name = "Arial"
            pl.font.size = Pt(10)
            pl.font.bold = is_hl
            pl.font.color.rgb = COLOR_TEXT_WHITE if is_hl else COLOR_TEXT_MUTED
            pl.space_after = Pt(6)

    # =========================================================================
    # SLIDE 12: 11 // ANDROID IMPLEMENTATION (Two High-Contrast Path Cards)
    # =========================================================================
    ...
    create_card(s12, Inches(0.65), CONTENT_TOP, c_w_half, CONTENT_H)
    tb_a1 = s12.shapes.add_textbox(Inches(0.95), CONTENT_TOP + Inches(0.30), c_w_half - Inches(0.60), CONTENT_H - Inches(0.50))
    ...
    for lead, rest in apk_steps:
        ps = tf_a1.add_paragraph()
        ps.text = "• "
        ps.font.name = "Arial"
        ps.font.size = Pt(10.5)
        ps.font.bold = True
        ps.font.color.rgb = COLOR_GOLD
    ...
    create_card(s12, c_right_x, CONTENT_TOP, c_w_half, CONTENT_H)
    tb_a2 = s12.shapes.add_textbox(c_right_x + Inches(0.30), CONTENT_TOP + Inches(0.30), c_w_half - Inches(0.60), CONTENT_H - Inches(0.50))
    ...
    # =========================================================================
    # SLIDE 13: 12 // PRODUCTION TIMELINE (4 Proportional Phase Cards)
    # =========================================================================
    ...
    for i, (pnum, pstat, ptitle, pbullets) in enumerate(phases_data):
        c_x = Inches(0.65) + i * (c_w4 + gap4)
        create_card(s13, c_x, CONTENT_TOP, c_w4, CONTENT_H)

        tb_ph = s13.shapes.add_textbox(c_x + Inches(0.25), CONTENT_TOP + Inches(0.25), c_w4 - Inches(0.50), CONTENT_H - Inches(0.40))
    ...
    # =========================================================================
    # SLIDE 14: 13 // CLOSING VISION (Center Card with Full Alignment)
    # =========================================================================
    ...
    create_card(s14, Inches(0.65), CONTENT_TOP, c_bar_w, CONTENT_H)
    tbc14 = s14.shapes.add_textbox(Inches(1.15), CONTENT_TOP + Inches(0.35), c_bar_w - Inches(1.00), CONTENT_H - Inches(0.60))
    ...
    # Save to Aligned and Restyled
    out_aligned = "Signify_iQOO_Hackathon_2026_Aligned.pptx"
    prs.save(out_aligned)
    print(f"Aligned presentation saved successfully to: {out_aligned}")

    try:
        prs.save("Signify_iQOO_Hackathon_2026_Theme_Restyled.pptx")
        print("Updated: Signify_iQOO_Hackathon_2026_Theme_Restyled.pptx")
    except Exception as e:
        print(f"Note: Theme_Restyled.pptx file lock ({e}). Saved as {out_aligned}!")

    try:
        prs.save("Signify_iQOO_Hackathon_2026_Final.pptx")
        print("Updated: Signify_iQOO_Hackathon_2026_Final.pptx")
    except Exception as e:
        print(f"Final.pptx notice: {e}")
